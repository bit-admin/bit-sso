// Cloudflare Worker for BIT SSO & yanhekt.cn Token Retrieval (Cookie Fix Version)
...
/**
 * 加密函数
 * @param {string} cryptoKey - 从页面获取的Base64编码的密钥
 * @param {string} password - 明文密码
 * @returns {string} 加密后的密码
 */
function encryptPassword(cryptoKey, password) {
    if (!cryptoKey) throw new Error("加密密钥(cryptoKey)缺失，无法加密密码。");
 ...
 * @param {string} findKey - 用于定位参数的唯一标识符, e.g., `id="login-croypto"`
 * @returns {string} 提取到的参数值
 */
...
/**
 * 从URL字符串中解析Token
 * @param {string} urlString - 包含token参数的URL
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (request.method !== 'POST') {
            return new Response('请求方法无效，请使用POST。', { status: 405 });
        }
        ...
        try {
            ...
            return new Response('请求体无效，请提供JSON格式的用户名和密码。', { status: 400 });
        }

        const { username, password } = credentials;
        if (!username || !password) {
            return new Response('用户名或密码缺失。', { status: 400 });
        }
        ...
        try {
            ...
            const initResponse = await fetch(`${API_LOGIN_PAGE}?service=${encodeURIComponent(serviceUrl)}`);
            
            const initialCookiesHeader = initResponse.headers.get('set-cookie');
            if (!initialCookiesHeader) throw new Error("未能获取到初始Cookie (JSESSIONID)。");
            sessionCookies = initialCookiesHeader.split(',').map(c => c.split(';')[0]).join('; ');
            ...
            const loginResponse = await fetch(`${API_LOGIN_ACTION}?service=${encodeURIComponent(serviceUrl)}`, {
                method: 'POST',
                headers: {
            ...
            if (loginResponse.status !== 302) {
                const errorData = await loginResponse.json().catch(() => ({ message: "登录失败，无法解析错误响应。" }));
                throw new Error(`SSO登录失败: ${errorData.message}`);
            }

            // --- 阶段 3: 跟随第一次重定向 (Ticket验证) ---
            ...
            // 关键修正: 捕获并合并所有SSO成功后设置的Cookie
            const ssoSuccessCookiesHeader = loginResponse.headers.get('set-cookie');
            if (ssoSuccessCookiesHeader) {
                const newCookies = ssoSuccessCookiesHeader.split(',').map(c => c.split(';')[0]).join('; ');
                sessionCookies += '; ' + newCookies;
            }

            const secondResponse = await fetch(firstRedirectLocation, {
                headers: {
                    // 关键修正: 在请求中发送所有累积的Cookie
                    'Cookie': sessionCookies
                },
                redirect: 'manual' // 关键: 继续手动处理重定向
            });
