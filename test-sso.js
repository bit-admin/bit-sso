...
function encryptPassword(cryptoKey, password) {
    ...
    // 关键修正：从页面获取的密钥本身是Base64编码的，需要按Base64解析
...
class SSOHandler {
    ...
    async initContext() {
        ...
        const response = await this.axiosInstance.get(API_LOGIN_PAGE, {
            params: { service: this.serviceUrl }
        });
        ...
        this.context.cryptoKey = this.getHtmlParam(response.data, CRYPTO_KEY_ID);
        this.context.executionKey = this.getHtmlParam(response.data, EXECUTION_KEY_ID);
    ...
    async doLogin(username, password) {
        ...
        try {
            ...
            if (error.response) {
                const errorReason = error.response.data?.message || `请求失败，状态码: ${error.response.status}`;
                return { success: false, reason: errorReason };
            }
            throw error;
        }
